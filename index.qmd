...
-High altitude affect baseball performance because it is harder to breathe at high altitude, so in turn yes Coors Field is affected by high altitude.
:::
...
there are 4,589 games in the dataset. We aggregate individual games into stadium averages before we start the visualization process to get a better understanding of the data and to make it more manageable.
...
- Horizontal bar chart is better than a scatter plot for this data because it is easier to compare the data and to see the differences between the stadiums.
- When would you choose a vertical bar chart over horizontal?
- Vertical bar chart is better than a horizontal bar chart when the data is more categorical and the data is more ordinal.

**Question 2: Aesthetic Mappings**
- What does the color highlighting accomplish in Approach 4?
- The color highlighting accomplishes in Approach 4 is to highlight the Colorado Rockies as the most run-friendly ballpark in 2010.
- How does position (x/y) compare to color for encoding data?
- Position (x/y) compare to color for encoding data is that position is used to encode the data and color is used to highlight the data.
...
- The elements that make this visualization suitable for a business presentation are the title, labels, legend, and annotation.
...
# Two-facet visualization: total runs by stadium for 2010 and 2021
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 8), sharey=True)

# Use consistent stadium order (sorted by 2010 runs) so both facets align
stadium_order = comparison_data.sort_values('runs_2010', ascending=True)
stadiums = stadium_order['home'].values
runs_2010 = stadium_order['runs_2010'].values
runs_2021 = stadium_order['runs_2021'].values

# Highlight Colorado (COL) in both facets
highlight_color = "darkorchid"
other_color = "lightgrey"
colors_2010 = [highlight_color if s == "COL" else other_color for s in stadiums]
colors_2021 = [highlight_color if s == "COL" else other_color for s in stadiums]

# Facet 1: 2010
ax1.barh(stadiums, runs_2010, color=colors_2010)
ax1.set_title("2010", fontsize=14, fontweight='bold')
ax1.set_xlabel("Average Total Runs Per Game", fontsize=11)
ax1.set_ylabel("Stadium (Home Team)", fontsize=11)
ax1.set_xlim(0, max(runs_2010) * 1.1)
ax1.tick_params(axis='y', labelsize=9)
ax1.grid(True, alpha=0.3)

# Facet 2: 2021
ax2.barh(stadiums, runs_2021, color=colors_2021)
ax2.set_title("2021", fontsize=14, fontweight='bold')
ax2.set_xlabel("Average Total Runs Per Game", fontsize=11)
ax2.set_ylabel("")
ax2.set_xlim(0, max(runs_2021) * 1.1)
ax2.tick_params(axis='y', labelsize=9)
ax2.grid(True, alpha=0.3)

# Legend (shared)
colorado_patch = plt.Rectangle((0, 0), 1, 1, color=highlight_color, label="Colorado (COL)")
other_patch = plt.Rectangle((0, 0), 1, 1, color=other_color, label="Other Stadiums")
fig.legend(handles=[colorado_patch, other_patch], loc='upper center', ncols=2, frameon=True, bbox_to_anchor=(0.5, 0.02))

fig.suptitle("Total Runs by Stadium: 2010 vs 2021", fontsize=16, fontweight='bold', y=1.02)
...
- The two-facet visualization that shows the total runs for 2010 and 2021 for each stadium in a single figure is a good visualization because it shows the total runs for 2010 and 2021 for each stadium in a single figure and it highlights the Colorado Rockies as the most run-friendly ballpark in 2010.

**Question 2: Explanation of the Visualization**
- The two-facet figure shows average total runs per game at each stadium in 2010 (left) and 2021 (right), using the same stadium order (by 2010 runs) so panels are directly comparable. Colorado (COL) is highlighted in purple in both facets. In 2010, Colorado leads all stadiums in runs per game; in 2021 we can see whether that pattern holds or how other stadiums compare. Shared y-axis and matching styling make it easy to see which stadiums are run-heavy in each year and whether COL remains an outlier.
- Does AI come to the right conclusion?  If not, why not?
- The visualization supports the conclusion when Colorado is clearly the highest (or among the highest) in both panels; if in 2021 another stadium exceeds COL, the same figure shows that and the conclusion would need to be updated accordingly.
:::
